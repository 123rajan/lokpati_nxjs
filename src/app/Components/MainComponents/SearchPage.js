...
import Both from "../../Components/ChildComponent/SideBarComponents/Both";
      ...
      <div className="w-[97%] md:w-[90%]">
        {loading ? (
          ...
          <div className="w-full grid grid-cols-6">
            <div className="col-span-6 min-h-[80vh] w-full mr-5 md:h-full  lg:col-span-4 flex flex-wrap py-4 px-3 justify-start gap-4">
              <div className="flex mt-7 w-full gap-2 border-b-2 border-[#660f22] items-center h-[80px]">
                {/* <Input.Group compact> */}
                <Input
                  value={inputValue}
                  onChange={(e) => setInputValue(e.target.value)}
                  onKeyDown={handleKeyDown}
                  className="text-xl text-mukta text-[#0b1378] max-w-[90%] px-3 py-4 border-none"
              ...
              <div className="flex flex-col min-h-[80%] gap-[15px] sm:gap-[30px] w-full">
                {allBlogs.slice(0, itemsToShow).map((blog) => (
                  <div
                    key={blog.id}
                    className="w-[90%] h-[130px] flex items-center py-[15px] px-3 rounded-md "
                    style={{ boxShadow: "0 0 5px #535454" }}
                  >
                    <SmallCardContentRight
                      title={blog.news_title}
                      sub_title={blog.news_sub_title}
                      image={`https://cms.lokpati.com${blog.image}`}
                      id={blog.id}
                      textBlack="true"
